...
from models import BabysitterSkill, ChildrensNeeds, NeedSkill, SpecialSkill, SpecialNeed, Contacted, Babysitter, Children, Parent, ParentsChildrens
...
query = session.query(
    BabysitterSkill.babysitterid, ChildrensNeeds.childid, SpecialSkill.skillname, SpecialNeed.needname, ParentsChildrens.parentid
).join(
    Babysitter, BabysitterSkill.babysitterid == Babysitter.babysitterid
).join(
    SpecialSkill, BabysitterSkill.skillid == SpecialSkill.skillid
).join(
    NeedSkill, SpecialSkill.skillid == NeedSkill.skillid
).join(
    SpecialNeed, NeedSkill.needid == SpecialNeed.needid
).join(
    ChildrensNeeds, SpecialNeed.needid == ChildrensNeeds.needid
).join(
    Children, ChildrensNeeds.childid == Children.childid
).join(
    ParentsChildrens, Children.childid == ParentsChildrens.childid
)
