...
from models import BabysitterSkill, ChildrensNeeds, NeedSkill, SpecialSkill, SpecialNeed, Contacted
...
query = session.query(
    BabysitterSkill, ChildrensNeeds, SpecialSkill, SpecialNeed
).join(
...
for babysitter_skill, children_need, special_skill, special_need in query.all():
    results.append({
        "babysitter_skill": special_skill.skillname,
        "child_need": special_need.needname
    })
