...
def create_input_data_for_parent(parentid):
    ...
    for babysitter in babysitters:
        ...
        Childs_Needs_Totalnum = sum(len(child.needs_association) for child in parent.childrens)
        ...
        babysitter_data['Favorites_totalnum'] = sum(1 for favorite in dal.get_all(models.Favorite) if favorite.babysitterid == babysitter.id)

        # Calculate the total number of reviews received by the babysitter
        babysitter_data['Babysitter_Total_Reviews_got'] = sum(1 for review in dal.get_all(models.Review) if review.reviewedid == babysitter.id)
...
def get_recommendations_for_parent(parentid):
    ...
    predicted_likelihood = nb_model.predict_proba(X)[:, 1]  # Assuming 1 is the label for "contacted"
    
    # Create a DataFrame with babysitter IDs and their corresponding likelihoods
    recommendations = pd.DataFrame({
        'Babysitterid': babysitter_ids,
